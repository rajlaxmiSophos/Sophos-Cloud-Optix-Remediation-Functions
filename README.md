...
|AWS|S3- Serverless Storage Protection| |Remediates detected malware files by deleting the file from the S3 bucket or quarantining the file as per the configured option. Malware files present in serverless storage negatively impact the security posture of an organisation.|
